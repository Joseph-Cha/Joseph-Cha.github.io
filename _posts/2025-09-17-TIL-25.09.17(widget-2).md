...
곧 build 메서드를 다시 한번 호출해 달라는 요청

-> 이것을 `setState()` 가능하다.
-> `setState`를 호출하면 build 함수가 다시 한번 호출되는 형태이다.
...
| `initState()`(중요) | `State`가 처음 초기화될 때 1회 호출 | 초기 상태 설정, 리스너 등록 |
| `didChangeDependencies()` | `initState()` 이후 또는 `InheritedWidget`이 변경되었을 때 | 의존성 관련 값 접근 가능 |
| `build()`(중요) | 위젯이 처음 그려질 때, 또는 `setState()`/`didUpdateWidget()` 호출 후 | UI 구성 |
...
| `dispose()`(중요) | 위젯이 완전히 트리에서 제거될 때 | 리소스 해제, 컨트롤러/리스너 정리 |
...
- **`expands`**
    ...
    - `maxLines: null`과 함께 반드시 사용해야 동작함
- **`maxLines`**
    - **줄 수 제한**
    - null 일 땐 줄 수 제한 없음
- **`onChanged`**
...
* 입력 초기값 설정, clear 등 다양한 작업 가능
* 더이상 사용하지 않을 때 dispose해줘야 한다(메모리 누수 방지).
...
- `Scaffold` 의 `bottomNavigationBar` 속성에 사용
...
* 홈 위젯에서 화면의 구조를 잡을 Scaffold 위젯을 생성
* AppBar 영역 구현
  * Title 텍스트 생성
* BottomSheet 영역 구현
  * TextField 생성
            ...
            * children
              * Container
                ...
                * decoration: BoxDecoration
                  ...
                  * shape: Box의 모양
              * SizedBox
              * Expanded: 자동으로 다음 칸으로 넘어가기 위해서
                * Text
              * Sized Box(끝 여백)
      * SizedBox

## 3-7: 투두앱 만들기 - StatefulWidget CRUD 구현
